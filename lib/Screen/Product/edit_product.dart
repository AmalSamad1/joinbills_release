    ...
    if (kIsWeb) {
      try {
        ...
        if (bytesFromPicker != null && bytesFromPicker.isNotEmpty) {
          ...
          img.Image? image = img.decodeImage(bytesFromPicker);

          // Convert image to JPG format
          Uint8List? jpgBytes = img.encodeJpg(image!, quality: 85);

          // Upload the image to Firebase Storage
          var snapshot = await FirebaseStorage.instance
              .ref('Profile Picture/${DateTime.now().millisecondsSinceEpoch}.jpg')
              .putData(jpgBytes!);
