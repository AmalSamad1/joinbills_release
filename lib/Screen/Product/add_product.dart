...
class _AddProductState extends State<AddProduct> {
  ...
  double extractGstRate(String gstString) {
    // Regular expression to match the percentage value
    final RegExp regex = RegExp(r'(\d+\.?\d*)%');
    final match = regex.firstMatch(gstString);

    if (match != null) {
      // Parse the matched percentage value as a double
      return double.tryParse(match.group(1) ?? '0') ?? 0.0;
    } else {
      return 0.0; // Default to 0% if no match is found
    }
  }
  double calculateGstAmount(double salePrice, String gstType) {
    double gstRate = extractGstRate(gstType); // Get the GST rate from the string
    return salePrice * (gstRate / 100); // Calculate the GST amount
  }
                                                          ...
                                                          return
                                                            InputDecorator(
                                                              ...
                                                              DropdownButtonHideUnderline(
                                                                child: DropdownButton<String>(
                                                                  value: gstSelect,
                                                                  items: allGstList.map((String items) {
                                                                    return DropdownMenuItem(
                                                                      value: items,
                                                                      child: Text(items),
                                                                    );
                                                                  ...
                                                                  onChanged: (String? value) {
                                                                    setState(() {
                                                                      if (allGstList.contains(value)) {
                                                                        gstSelect = value!;
                                                                        toast(gstSelect);
                                                                      } else {
                                                                        toast('Invalid GST selection');
                                                                      }
                                                                    });
                                                                  },
                                                                ),
                                                              ...
                                                              setState(() {
                                                                ...
                                                                productQuantityController
                                                                    ...
                                                                    ? productQuantityController
                                                                        ...
                                                                        '1'
                                                                    : productQuantityController
                                                                        .text = (int.parse(productQuantityController.text) +
                                                                            1)
                                                                        .toString();
                                                              });
                                                              productSerialNumberController
                                                        ...
                                                        if (validateAndSave()) {
                                                          try {
                                                            EasyLoading.show(
                                                                ...
                                                                dismissOnTap:
                                                                    false);
                                                            double salePrice = double.tryParse(productSalePriceController.text) ?? 0.0;
                                                            String gstType = gstSelect ?? ''; // Assuming gstSelect holds the selected GST option
                                                            double gstAmount = calculateGstAmount(salePrice, gstType);
